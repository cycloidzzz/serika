...
def solve_use(block: List[JsonType]):
...
def solve_def(block: List[JsonType]):
    defs: Set[str] = set()
    for instr in block:
        if 'op' in instr:
            if 'dest' in instr:
...
def live_variable_analysis(named_blocks: Dict[str, List[JsonType]]):
    ...
    while True:
        ...
        for name, block in named_blocks.items():
            fact_union = union(
                    [block_in[v] for v in succ[name]]
            )
            if len(fact_union ^ block_out[name]) > 0:
                changed = True
            
            block_out[name] = fact_union
            block_in[name] = (block_out[name] - block_def[name]) | block_use[name]
